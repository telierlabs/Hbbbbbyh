    ...
    <Link to={`/news/${article.id}`} style={{ textDecoration: 'none', display: 'block' }}>
      <div className="tn-hero-media" style={{ position: 'relative', width: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: '24px', left: '20px', right: '20px' }}>
          <h1 style={{ ...m, fontSize: 'clamp(22px,4vw,34px)', fontWeight: 700, color: '#fff', lineHeight: 1.25, letterSpacing: '-0.01em', marginBottom: '10px' }}>
            {article.title}
          </h1>
...
const Home: React.FC<HomeProps> = ({ articles, searchQuery = '' }) => {
  ...
  return (
    <>
      ...
      <style>{`
        /* AI Insight — mobile: compact, di bawah hero, tinggi dibatasi */
        .tn-insight {
          margin: -24px 20px 0;
          position: relative;
          z-index: 2;
        ...
        /* Desktop: kontainer lebih lebar, Top Stories 4 kolom, hero jadi landscape,
           AI Insight jadi floating card di samping judul (bukan strip full-width) */
