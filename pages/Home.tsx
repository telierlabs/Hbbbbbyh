...
const AIInsight: React.FC<{ article: NewsArticle }> = ({ article }) => {
  ...
  return (
    <div className="tn-insight">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: '10px', marginBottom: '12px', borderBottom: '1px solid #e2e2e2' }}>
        <span style={{ ...mono, fontSize: '9px', letterSpacing: '0.15em', color: '#8a8a8a', textTransform: 'uppercase' }}>AI Insight</span>
      </div>
      <ul className="tn-insight-list" style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '8px', margin: 0, padding: 0 }}>
        {article.aiInsight.map((point, i) => (
          <li key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
            <span style={{ ...mono, fontSize: '10px', color: '#aaaaaa', flexShrink: 0 }}>{String(i + 1).padStart(2, '0')}</span>
            <span style={{ ...m, fontSize: '12px', color: '#333333', lineHeight: 1.55 }}>{point}</span>
          </li>
        ))}
      </ul>
    </div>
...
const Home: React.FC<HomeProps> = ({ articles, searchQuery = '' }) => {
  ...
  return (
    <>
      ...
      <style>{`
        /* Top Stories — MOBILE: cuma 1 item, layout horizontal (gambar kiri, teks kanan) */
        .tn-top-grid { display: flex; flex-direction: column; }
        .tn-top-item { display: flex; flex-direction: row; align-items: center; gap: 12px; }
        .tn-top-item:nth-child(n+2) { display: none; }
        .tn-top-item-media { width: 92px; height: 72px; }
