  ...
  <div className="tn-hero" style={{ position: 'relative', background: '#000' }}>
    <Link to={`/news/${article.id}`} style={{ textDecoration: 'none', display: 'block' }}>
      <div className="tn-hero-media" style={{ position: 'relative', width: '100%', overflow: 'hidden' }}>
        ...
        <div className="tn-hero-text" style={{ position: 'absolute', bottom: '24px', left: '20px', right: '20px' }}>
...
// Mobile: block biasa di antara hero & Top Stories (tidak menempel/menutupi foto hero), bisa dilipat/dibuka.
    ...
    <div className="tn-insight">
      <div
        ...
        style={{
          ...
          borderBottom: expanded ? '1px solid #e2e2e2' : 'none',
        }}
      >
        <span style={{ ...m, fontSize: '15px', fontWeight: 700, color: '#1a1a1a' }}>AI Insight</span>
        <svg
          width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#777" strokeWidth={2.4}
      ...
      {expanded && (
        <ul className="tn-insight-list" style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '12px', margin: 0, padding: 0 }}>
          {article.aiInsight.map((point, i) => (
            <li key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
              <span style={{ ...m, fontSize: '13px', fontWeight: 700, color: '#999999', flexShrink: 0 }}>{i + 1}</span>
              <span style={{ ...m, fontSize: '14px', color: '#333333', lineHeight: 1.6 }}>{point}</span>
            </li>
          ))}
        </ul>
      )}
...
// ── TOP STORIES: mobile cuma 1 item (horizontal) + link "Lihat Semua", desktop grid 4 kolom ──
const TopStories: React.FC<{ articles: NewsArticle[] }> = ({ articles }) => {
  if (articles.length === 0) return null;
  return (
    <div style={{ padding: '28px 0 8px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        <span style={{ ...m, fontSize: '17px', fontWeight: 700, color: THEME.text }}>Top Stories</span>
        <Link to="/top-stories" style={{ ...mono, fontSize: '10px', letterSpacing: '0.1em', color: THEME.muted, textDecoration: 'none' }}>Lihat Semua</Link>
      </div>
      <div className="tn-top-grid">
        {articles.map(article => (
          <Link key={article.id} to={`/news/${article.id}`} className="tn-top-item" style={{ textDecoration: 'none' }}>
            <div className="tn-top-item-media" style={{ position: 'relative', overflow: 'hidden', borderRadius: '10px', background: '#f0f0f0', flexShrink: 0 }}>
              <img
                src={article.imageUrl}
                alt={article.title}
                referrerPolicy="no-referrer"
            ...
            <div className="tn-top-item-text">
...
const Home: React.FC<HomeProps> = ({ articles, searchQuery = '' }) => {
  ...
  return (
    <>
      ...
      <style>{`
        /* Top Stories — MOBILE: cuma 1 item, layout horizontal (gambar kiri lebih besar, teks kanan) */
        .tn-top-grid { display: flex; flex-direction: column; }
        .tn-top-item { display: flex; flex-direction: row; align-items: center; gap: 14px; }
        .tn-top-item:nth-child(n+2) { display: none; }
        .tn-top-item-media { width: 132px; height: 100px; }
        .tn-top-item-text { flex: 1; min-width: 0; }

        /* AI Insight — MOBILE: block biasa di atas Top Stories, abu cerah, tinggi menyesuaikan isi */
        .tn-insight {
          position: static;
          margin: 16px 20px 0;
          background: #f7f7f7;
        ...
        /* Desktop: kontainer lebih lebar, Top Stories jadi grid 4 kolom (vertikal), hero jadi landscape,
           judul hero dibatasi lebarnya biar ga nabrak card, AI Insight jadi floating card
           di kanan-atas hero — tinggi DIBATASI biar ga nutupin Top Stories di bawahnya */
        @media (min-width: 900px) {
          .tn-container { max-width: 1080px !important; }
          .tn-hero-media { padding-top: 42% !important; }

          .tn-top-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px 12px; }
          .tn-top-item { display: block; }
          .tn-top-item:nth-child(n+2) { display: block; }
          .tn-top-item-media { width: 100%; height: 0; padding-top: 72%; margin-bottom: 8px; }

          .tn-feed-col { columns: 2; column-gap: 28px; }
          .tn-feed-col > * { break-inside: avoid; }

          .tn-hero-title { font-size: 26px !important; }
          .tn-hero-text { max-width: 56% !important; right: auto !important; }
          ...
          .tn-insight::-webkit-scrollbar { width: 4px; }
          .tn-insight::-webkit-scrollbar-thumb { background: #ccc; border-radius: 4px; }
        }
