      ...
      <ul className="tn-insight-list" style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '8px', margin: 0, padding: 0 }}>
        {article.aiInsight.map((point, i) => (
          <li key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
            ...
            <span style={{ ...m, fontSize: '12px', color: '#ddd', lineHeight: 1.55 }}>{point}</span>
          </li>
        ))}
      </ul>
    </div>
...
const Home: React.FC<HomeProps> = ({ articles, searchQuery = '' }) => {
  ...
  return (
    <>
      ...
      <style>{`
        /* AI Insight — MOBILE: block biasa di atas Top Stories, tinggi menyesuaikan isi (no scroll) */
        .tn-insight {
          position: static;
          margin: 16px 20px 0;
          background: #2b2b2b;
        ...
        /* Desktop: kontainer lebih lebar, Top Stories 4 kolom, hero jadi landscape,
           ...
           di kanan-atas hero (di bawah header, tinggi menyesuaikan isi) */
