...
const Admin: React.FC<AdminProps> = ({ articles, onAddArticle, onDeleteArticle }) => {
  ...
  // State untuk popup notif setelah publish
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simpan artikel yang baru dipublish untuk popup notif
  ...
  const handleKirimNotif = async () => {
    ...
    setNotifResult(null);
    try {
      // Cari articleId dari articles list (artikel terbaru)
      const latestArticle = articles[0];
      const result = await sendNotification(
        `[BREAKING] ${lastPublished.title}`,
        lastPublished.summary,
        latestArticle?.id || '',
        lastPublished.imageUrl,
      );
      setNotifResult(`✅ Terkirim ke ${result.sent} subscriber`);
    } catch (err) {
      setNotifResult('❌ Gagal kirim notifikasi');
    } finally {
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full">
            <div className="text-center mb-6">
              ...
              <h3 className="text-xl font-bold mb-2">Artikel Berhasil Dipublish!</h3>
              <p className="text-gray-500 text-sm mb-1">"{lastPublished?.title}"</p>
              <p className="text-gray-400 text-sm">Mau kirim notifikasi ke semua subscriber?</p>
            </div>

            {notifResult && (
              <div className={`text-center py-3 px-4 rounded-xl text-sm font-medium mb-4 ${notifResult.startsWith('✅') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
                {notifResult}
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { setShowNotifPopup(false); setNotifResult(null); }}
                className="flex-1 py-3 border border-gray-200 rounded-2xl text-gray-600 font-medium hover:bg-gray-50 transition-all"
              >
                Tidak
              </button>
              <button
                onClick={handleKirimNotif}
                disabled={sendingNotif || !!notifResult?.startsWith('✅')}
                className="flex-2 flex-1 py-3 bg-black text-white rounded-2xl font-bold hover:bg-gray-800 transition-all disabled:opacity-50"
              >
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                            <label className="cursor-pointer bg-white px-3 py-1 rounded text-xs font-bold">
                              ...
                              <input type="file" accept="image/*"
                                onChange={(e) => handleSectionImageUpload(section.id, e)} className="hidden" />
                            </label>
                          </div>
                        </>
                      ) : (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <label className="block w-full px-4 py-3 bg-black text-white text-center rounded-xl cursor-pointer hover:bg-gray-800 transition-all">
                        ...
                        <input type="file" accept="video/*"
                          ...
                          className="hidden" disabled={section.uploadingVideo} />
                      </label>
                    </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {articles.map(article => (
              <tr key={article.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <button onClick={() => { if (window.confirm('Yakin ingin menghapus berita ini?')) { onDeleteArticle(article.id); } }}
