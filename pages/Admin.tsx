...
const IMGBB_API_KEY = import.meta.env.VITE_IMG_KEY || import.meta.env.IMG_KEY || '';
...
const Admin: React.FC<AdminProps> = ({ articles, onAddArticle, onDeleteArticle }) => {
  ...
  const handleThumbnailUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json.success) {
        setFormData(prev => ({ ...prev, imageUrl: json.data.display_url }));
      } else {
  ...
  const handleSectionImageUpload = async (sectionId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json.success) {
        setSections(sections.map(s => 
          s.id === sectionId ? { ...s, imageUrl: json.data.display_url, uploadingImage: false } : s
        ));
      } else {
        alert('Upload gambar gagal!');
        setSections(sections.map(s => 
  ...
  const handleSectionVideoUpload = async (sectionId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json.success) {
        setSections(sections.map(s => 
          s.id === sectionId ? { ...s, videoUrl: json.data.display_url, uploadingVideo: false } : s
        ));
      } else {
