...
interface AdminProps {
  ...
  onAddArticle: (article: NewsArticle) => Promise<string | null>;
...
// ⚠️ DEBUG VERSION - pakai alert() sementara buat cari tahu kenapa indexing gagal
const submitToIndexing = async (articleId: string, type: 'URL_UPDATED' | 'URL_DELETED' = 'URL_UPDATED') => {
  try {
    ...
    if (!res.ok) {
      alert('❌ Indexing gagal: ' + JSON.stringify(data));
    } else {
      alert('✅ Indexing sukses: ' + data.url);
    }
  } catch (err: any) {
...
const Admin: React.FC<AdminProps> = ({ articles, onAddArticle, onUpdateArticle, onDeleteArticle }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'edit' && editingId) {
        ...
        submitToIndexing(editingId, 'URL_UPDATED');
        ...
        const newId = await onAddArticle(newArticle);
        if (newId) {
          submitToIndexing(newId, 'URL_UPDATED');
          setLastPublishedId(newId);
        }
  ...
  const handleKirimNotif = async () => {
    ...
    try {
      const result = await sendNotification(`[BREAKING] ${lastPublished.title}`, lastPublished.summary, lastPublishedId || '', lastPublished.imageUrl);
  ...
  const handleDelete = (id: string) => {
    ...
    onDeleteArticle(id);
    submitToIndexing(id, 'URL_DELETED');
  };
          ...
          {articles.map(article => (
            <div key={article.id} style={{ background: '#fff', borderRadius: '14px', border: '1px solid #ececec', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', flexShrink: 0 }}>
                ...
                <button onClick={() => handleDelete(article.id)} style={{ padding: '5px 10px', border: '1px solid #ffecec', background: '#fff5f5', color: '#ff4444', borderRadius: '7px', fontSize: '10px', fontWeight: 700, cursor: 'pointer' }}>Hapus</button>
              </div>
            </div>
          ))}
        </div>
