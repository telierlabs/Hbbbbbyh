...
const Admin: React.FC<AdminProps> = ({ articles, onAddArticle, onDeleteArticle }) => {
  ...
  // Content sections (bisa multiple)
  const [sections, setSections] = useState<ContentSection[]>([
    { id: '1', text: '', imageUrl: '', videoUrl: '', uploadingImage: false, uploadingVideo: false }
  ]);
  ...
  // Upload video untuk section
  const handleSectionVideoUpload = async (sectionId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // Untuk video, kita pakai ImgBB juga (support video)
    setSections(sections.map(s => 
      s.id === sectionId ? { ...s, uploadingVideo: true } : s
    ));

    const data = new FormData();
    data.append('image', file); // ImgBB juga terima video dengan param 'image'
  ...
  // Update section video URL (untuk YouTube link)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Convert sections ke contentBlocks
    ...
    sections.forEach(section => {
      ...
      // Add video block (as text with video URL for now)
      if (section.videoUrl) {
        contentBlocks.push({
          type: 'text',
    ...
    const fullContent = sections.map(s => s.text).filter(t => t.trim()).join('\n\n');
    ...
    setSections([{ id: '1', text: '', imageUrl: '', videoUrl: '', uploadingImage: false, uploadingVideo: false }]);
    setIsAdding(false);
  };
