...
const NovelPages: React.FC<{ pages: ArticlePage[]; prefs: ReadingPrefs; onOpenViewer: (items: MediaItem[], index: number) => void }> = ({ pages, prefs, onOpenViewer }) => {
  ...
  // ✅ FIX: Hapus scrollIntoView — tidak ada auto scroll sama sekali
  const go = (i: number) => {
...
const ChatFullscreen: React.FC<{ article: NewsArticle; fullText: string; onClose: () => void }> = ({ article, fullText, onClose }) => {
  ...
  // ✅ FIX: Scroll di dalam container chat saja, bukan window
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* ✅ FIX: ref dipasang di sini untuk scroll internal */}
      <div ref={messagesContainerRef} style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        {/* ✅ FIX: messagesEndRef dihapus total */}
      </div>
