  ...
  return (
    <div
      ...
      onTouchEnd={e => {
        ...
        // Swipe down >80px dan lebih vertical dari horizontal → close
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderBottom: '1px solid #f0f0f0', background: '#fff', flexShrink: 0 }}>
        ...
        {/* Swipe indicator */}
        <div style={{ position: 'absolute', top: '6px', left: '50%', transform: 'translateX(-50%)', width: '36px', height: '4px', borderRadius: '2px', background: '#e0e0e0' }} />
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {messages.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px 20px' }}>
            ...
            <p style={{ fontSize: '11px', color: '#ccc', marginTop: '8px' }}>Geser ke bawah untuk menutup</p>
          </div>
        )}
        {messages.map((msg, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{ maxWidth: '80%', padding: '12px 16px', borderRadius: msg.role === 'user' ? '18px 18px 4px 18px' : '18px 18px 18px 4px', background: msg.role === 'user' ? '#111' : '#f5f5f5', color: msg.role === 'user' ? '#fff' : '#222', fontSize: '14px', lineHeight: 1.6 }}>
              {msg.content}
            </div>
          </div>
        ))}
      ...
      <div style={{ padding: '12px 16px', borderTop: '1px solid #f0f0f0', background: '#fff', display: 'flex', gap: '10px', alignItems: 'center', flexShrink: 0 }}>
        ...
        />
        <button onClick={handleSubmit} disabled={loading || !input.trim()} style={{ width: '44px', height: '44px', borderRadius: '50%', background: input.trim() ? '#111' : '#ddd', border: 'none', cursor: input.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
const NewsDetail: React.FC<NewsDetailProps> = ({ articles }) => {
  ...
  // ── FIX UTAMA: hapus `article` dari dependency array ──
  // Sebelumnya: [id, article] → setiap kali article re-render, scroll ke atas lagi
  // Sekarang: [id] → scroll ke atas HANYA saat pindah artikel (id berubah)
  useEffect(() => {
    window.scrollTo(0, 0);
    if (article) updateOGTags(article);
    return () => resetOGTags();
  }, [id]); // <-- FIXED: hanya [id], bukan [id, article]
