...
const ArticleCarousel: React.FC<{ items: CarouselItem[]; title: string; category: string; publishedAt: string }> = ({ items, title, category, publishedAt }) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '65vw', maxHeight: '520px', minHeight: '280px', overflow: 'hidden', background: '#000' }}>
      ...
      {items.length > 1 && <div style={{ position: 'absolute', top: '12px', right: '14px', fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', color: 'rgba(255,255,255,0.7)', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', padding: '3px 8px', borderRadius: '10px', zIndex: 3 }}>{idx + 1} / {items.length}</div>}
...
const NewsDetail: React.FC<NewsDetailProps> = ({ articles }) => {
  ...
  // Realtime views + increment
  useEffect(() => {
    if (!id) return;
    // Increment views (sekali per sesi)
    incrementViews(id);
    // Subscribe realtime
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                  <svg width="11" height="11" fill="none" stroke="#bbb" viewBox="0 0 24 24" strokeWidth={2}>
