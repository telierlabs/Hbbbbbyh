// NewsDetail.tsx - WITH DYNAMIC OG TAGS + CYLEN AI CHAT + COMMENTS
  ...
  return (
    <>
      ...
      <article className="max-w-2xl mx-auto px-4 py-8">
        <Link to="/" className="text-sm font-medium text-gray-400 hover:text-black transition-colors mb-6 inline-block">
          &larr; Kembali
        </Link>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {relatedArticles.map(related => (
                <Link key={related.id} to={`/news/${related.id}`} className="group">
                  ...
                  <h3 className="font-bold text-sm mt-1 line-clamp-2 group-hover:text-black transition-colors">{related.title}</h3>
      ...
      {showScrollButton && (
        <button onClick={scrollDown}
          className="fixed bottom-20 right-6 w-12 h-12 bg-black text-white rounded-full shadow-2xl hover:bg-gray-800 transition-all z-40 flex items-center justify-center text-xl"
