    ...
    <div style={{ border: '1px solid #ececec', borderRadius: '14px', overflow: 'hidden', marginTop: '12px' }}>
      ...
      {open && <div style={{ padding: '16px' }}>
        {loading && points.length === 0
          ? <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>{[0,200,400].map(d => <div key={d} style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#ddd', animation: `dot-pulse 1.2s ease ${d}ms infinite` }} />)}<span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', color: '#aaa', letterSpacing: '0.1em' }}>Menganalisis artikel...</span></div>
...
const RichTextBlock: React.FC<{ html: string; prefs: ReadingPrefs }> = ({ html, prefs }) => {
  ...
  return <p style={{ fontFamily: fontMap[prefs.font], fontSize: sizeMap[prefs.size], lineHeight: lhMap[prefs.lh], color: '#222', marginBottom: '14px', whiteSpace: 'pre-wrap' }}>{html}</p>;
};

// ── NOVEL PAGES ──
const NovelPages: React.FC<{ pages: ArticlePage[]; prefs: ReadingPrefs; onOpenViewer: (items: MediaItem[], index: number) => void }> = ({ pages, prefs, onOpenViewer }) => {
  const [idx, setIdx] = useState(0);
  const touchStartX = useRef(0);
  const contentRef = useRef<HTMLDivElement>(null);
  const go = (i: number) => { const next = Math.max(0, Math.min(pages.length - 1, i)); setIdx(next); setTimeout(() => contentRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50); };
            ...
            return (
              <div key={i} style={{ margin: '18px 0', borderRadius: '12px', overflow: 'hidden', background: '#000' }}>
                {block.content.includes('youtube.com') || block.content.includes('youtu.be')
                  ...
                  : <div style={{ position: 'relative', cursor: 'pointer' }} onClick={() => onOpenViewer(pageMediaItems, Math.max(0, mediaIdx))}>
  ...
  return (
    <>
      ...
      <article style={{ maxWidth: '680px', margin: '0 auto', padding: '0 20px 80px' }}>
        ...
        {/* VOICE READER — import dari components */}
