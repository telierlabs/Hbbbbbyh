...
// ── CAROUSEL ──
const ArticleCarousel: React.FC<{ items: CarouselItem[]; title: string; category: string; publishedAt: string }> = ({ items, title, category, publishedAt }) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '65vw', maxHeight: '520px', minHeight: '280px', overflow: 'hidden', background: '#000' }}>
      <div style={{ display: 'flex', height: '100%', transform: `translateX(-${idx * 100}%)`, transition: 'transform 0.45s cubic-bezier(0.4,0,0.2,1)' }}>
        {items.map((item, i) => (
          <div key={i} style={{ flexShrink: 0, width: '100%', height: '100%', position: 'relative' }}>
            {item.type === 'video'
              ? <video src={item.url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} autoPlay={i === idx} muted loop playsInline />
              : <img src={item.url} alt={title} referrerPolicy="no-referrer" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
          </div>
        ))}
      </div>
      ...
      {items.length > 1 && (
        <div style={{ position: 'absolute', top: '12px', right: '14px', fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', color: 'rgba(255,255,255,0.7)', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', padding: '3px 8px', borderRadius: '10px', zIndex: 3 }}>{idx + 1} / {items.length}</div>
      )}
...
const AIInsight: React.FC<{ article: NewsArticle; fullText: string }> = ({ article, fullText }) => {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'user', content: `Berikan 4 poin insight singkat dan tajam dari artikel berita ini dalam Bahasa Indonesia. Setiap poin maksimal 20 kata. Format: hanya JSON array of strings. Judul: ${article.title}. Konten: ${fullText.substring(0, 1500)}` }]
        })
      });
  ...
  useEffect(() => {
    if (!loaded.current && points.length === 0) { loaded.current = true; setTimeout(generate, 800); }
  }, []);
    ...
    {article.contentBlocks && article.contentBlocks.length > 0 ? (
      <div className="space-y-6">
        {article.contentBlocks.map((block, i) => (
          <div key={i}>
            {block.type === 'image' ? <div className="relative rounded-2xl overflow-hidden my-6"><img src={block.content} alt="" className="w-full object-cover" /></div>
...
const ChatFullscreen: React.FC<{ article: NewsArticle; fullText: string; onClose: () => void }> = ({ article, fullText, onClose }) => {
  ...
  useEffect(() => {
    // Auto focus input saat chat terbuka
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#fff', zIndex: 200, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderBottom: '1px solid #f0f0f0', background: '#fff', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: '10px', color: '#aaa', margin: 0, fontFamily: "'Share Tech Mono', monospace", letterSpacing: '0.08em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{article.title}</p>
        </div>
      </div>
      ...
      <div style={{ padding: '12px 16px', borderTop: '1px solid #f0f0f0', background: '#fff', display: 'flex', gap: '10px', alignItems: 'center', flexShrink: 0 }}>
        ...
        <button onClick={handleSubmit} disabled={loading || !input.trim()}
          style={{ width: '44px', height: '44px', borderRadius: '50%', background: input.trim() ? '#111' : '#ddd', border: 'none', cursor: input.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s', flexShrink: 0 }}>
...
const NewsDetail: React.FC<NewsDetailProps> = ({ articles }) => {
  ...
  return (
    <>
      ...
      {chatMode && (
        <ChatFullscreen article={article} fullText={fullText} onClose={() => setChatMode(false)} />
      )}

      {/* ARTIKEL NORMAL */}
      <ArticleCarousel items={carouselItems} title={article.title} category={article.category} publishedAt={article.publishedAt} />
      ...
      <article style={{ maxWidth: '680px', margin: '0 auto', padding: '0 20px 80px' }}>
        ...
        {/* 4. CYLEN AI CHAT — input yang kalau diklik buka fullscreen */}
        <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #f0f0f0' }}>
          ...
          {/* Input dummy — kalau diklik buka chat fullscreen */}
      ...
      {/* Scroll down button — sembunyikan kalau chat mode */}
      {!chatMode && (
