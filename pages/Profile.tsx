  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (!currentUser) {
        ...
        setUser(currentUser);
        // Cek status notif dari browser + localStorage
    ...
    try {
      if (!notifications) {
        ...
        if (permission !== 'granted') {
          alert('Izin notifikasi ditolak. Aktifkan manual di pengaturan browser.');
          return;
        }
        ...
        if (messaging && savedToken) {
          try {
            ...
            console.warn('Token delete error:', e);
          }
        }
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: '#555', margin: '3px 0 0' }}>
              {notifications ? 'Aktif — kamu akan menerima notif berita penting' : 'Nonaktif — aktifkan untuk terima breaking news'}
            </p>
          </div>
          <button
            ...
            style={{
              ...
              borderRadius: 12, border: 'none', cursor: notifLoading ? 'wait' : 'pointer',
