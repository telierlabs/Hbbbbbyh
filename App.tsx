...
import { fetchArticles, addArticle, deleteArticle, updateArticle } from './services/articleService';
...
const AppContent = () => {
  ...
  const isTikTok     = location.pathname === '/tiktok';
  ...
  const handleAddArticle = async (article: NewsArticle): Promise<string | null> => {
    const newId = await addArticle(article);
    if (newId) await loadArticles();
    return newId ?? null;
  };

  const handleUpdateArticle = async (articleId: string, updates: Partial<NewsArticle>) => {
  ...
  const isDarkPage = isHomePage || isInstagram || isTikTok || isShopPage;

  return (
    <div className="min-h-screen flex flex-col" style={{ background: isDarkPage ? '#000' : '#fff' }}>
      ...
      <main className={`flex-grow ${!isHomePage && !isInstagram && !isTikTok ? 'pt-16 md:pt-20' : ''}`}>
        <Routes>
          ...
          <Route path="/top-stories"     element={<TopStoriesPage articles={articles} />} />
          <Route path="/instagram"       element={<Instagram articles={articles} />} />
          <Route path="/tiktok"          element={<TikTok articles={articles} />} />
          ...
          <Route path="/admin"           element={<Admin articles={articles} onAddArticle={handleAddArticle} onUpdateArticle={handleUpdateArticle} onDeleteArticle={handleDeleteArticle} />} />
          <Route path="/admin/login"     element={<AdminLogin />} />
          <Route path="*"                element={<Navigate to="/" replace />} />
        </Routes>
      </main>

      {!isNewsDetail && !isInstagram && !isTikTok && <Footer />}
      <NotificationBanner />
    </div>
  );
};
