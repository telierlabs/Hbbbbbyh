...
// ✅ BmBtn — Firebase-aware bookmark button
const BmBtn: React.FC<{ article: NewsArticle }> = ({ article }) => {
  ...
  // Cek status bookmark saat userId atau articleId berubah
  useEffect(() => {
    isBookmarked(userId, article.id).then(setSaved);
  }, [userId, article.id]);

  // Sync dari event localStorage (fallback)
  useEffect(() => {
    const sync = () => {
