...
import { fetchViews, formatViews, saveBookmark, removeBookmark, isBookmarked, BookmarkItem } from '../services/articleService';
...
// ✅ BmBtn — di-export supaya bisa dipakai di Home.tsx
export const BmBtn: React.FC<{ article: NewsArticle }> = ({ article }) => {
  ...
  useEffect(() => {
    isBookmarked(userId, article.id).then(setSaved);
  }, [userId, article.id]);

  useEffect(() => {
    const sync = () => {
      if (!userId) isBookmarked(null, article.id).then(setSaved);
    };
  ...
  const toggle = async (e: React.MouseEvent) => {
    ...
    // ✅ Optimistic update — langsung toggle visual tanpa tunggu async
  ...
  return (
    <button
      ...
      style={{
        ...
        border: saved ? '1px solid #fff' : '1px solid #2a2a2a',
        background: saved ? '#222' : 'transparent',
