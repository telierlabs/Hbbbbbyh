...
const Badge: React.FC<{ cat: string }> = ({ cat }) => (
  <span style={{
    ...mono, fontSize: '7px', letterSpacing: '0.2em', textTransform: 'uppercase',
    color: '#e0e0e0', padding: '2px 8px', borderRadius: '3px',
...
const BmBtn: React.FC<{ article: NewsArticle }> = ({ article }) => {
  ...
  return (
    <button onClick={toggle} style={{
      ...
      cursor: 'pointer', color: saved ? '#fff' : '#444', transition: 'all 0.2s', flexShrink: 0,
    }}>
      <svg width="11" height="11" fill={saved ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
  ...
  <Link to={`/news/${article.id}`} style={{ textDecoration: 'none', display: 'block', marginBottom: '10px' }} className="group">
    <div style={{ background: '#080808', border: '1px solid #161616', borderRadius: '14px', overflow: 'hidden', transition: 'border-color 0.25s' }}
      ...
      <div style={{ position: 'relative', width: '100%', paddingTop: '56%', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: '14px', left: '14px', right: '14px', ...m, fontSize: '17px', fontWeight: 600, color: '#fff', lineHeight: 1.3, letterSpacing: '-0.01em' }}>
          {article.title}
        </div>
      </div>
  ...
  <Link to={`/news/${article.id}`} style={{ textDecoration: 'none', display: 'block', height: '100%' }} className="group">
    <div style={{ background: '#080808', border: '1px solid #161616', borderRadius: '12px', overflow: 'hidden', transition: 'border-color 0.25s', height: '100%' }}
      ...
      <div style={{ padding: '10px 10px 12px' }}>
        <div style={{ ...m, fontSize: '12px', fontWeight: 500, color: '#ddd', lineHeight: 1.4, marginBottom: '8px', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {article.title}
        </div>
  ...
  <Link to={`/news/${article.id}`} style={{ textDecoration: 'none', display: 'block', marginBottom: '10px' }} className="group">
    <div style={{ display: 'flex', background: '#080808', border: '1px solid #161616', borderRadius: '12px', overflow: 'hidden', transition: 'border-color 0.25s' }}
      ...
      <div style={{ flex: 1, padding: '12px 14px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div style={{ ...m, fontSize: '13px', fontWeight: 500, color: '#ddd', lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {article.title}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '8px' }}>
          <span style={{ ...mono, fontSize: '8px', color: '#444' }}>{formatDate(article.publishedAt)} · {getRelativeTime(article.publishedAt)}</span>
          <BmBtn article={article} />
        </div>
      </div>
    </div>
    ...
    <Link to={`/news/${article.id}`} style={{ textDecoration: 'none', display: 'block', marginBottom: '10px' }} className="group">
      <div style={{ background: '#080808', border: '1px solid #161616', borderRadius: '12px', overflow: 'hidden', transition: 'border-color 0.25s' }}
        ...
        <div style={{ padding: '14px 16px' }}>
          ...
          <div style={{ ...m, fontSize: '14px', fontWeight: 500, color: '#e0e0e0', lineHeight: 1.4, marginBottom: '10px' }}>
            {article.title}
          </div>
