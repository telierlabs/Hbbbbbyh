...
// ✅ DEBUG — hapus setelah jalan
console.log('🔑 GEMINI KEYS:', GEMINI_KEYS.map((k, i) => `KEY${i+1}: ${k ? k.substring(0,8)+'...' : 'KOSONG'}`));
...
- Maksimal 3 paragraf`;
  ...
  for (let i = 0; i < GEMINI_KEYS.length; i++) {
    const key = getKey();
    if (!key) {
      console.warn(`⚠️ Key ${i+1} KOSONG, skip`);
      continue;
    }
    try {
      console.log(`🚀 Mencoba key ${i+1}: ${key.substring(0,8)}...`);
      ...
      // ✅ DEBUG — log status & response error
      console.log(`📡 Status key ${i+1}:`, res.status);
      if (!res.ok) {
        const errData = await res.json();
        console.error(`❌ Error key ${i+1}:`, JSON.stringify(errData));
        continue;
      }
      ...
      if (text) {
        console.log(`✅ Berhasil pakai key ${i+1}`);
        return text;
      }
    } catch (err) {
      console.warn(`❌ Key ${i+1} exception:`, err);
    }
  }
