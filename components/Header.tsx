...
import {
  BookmarkItem, fetchBookmarks, removeBookmark, subscribeToBookmarks
...
const Header: React.FC<HeaderProps> = ({ onMenuToggle, onSearch }) => {
  ...
  const syncBookmarks = useCallback(async (uid: string | null) => {
    const bms = await fetchBookmarks(uid);
    setBookmarks(bms);
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      ...
      setUserId(uid);
      if (uid) {
        const unsubBm = subscribeToBookmarks(uid, (bms) => {
          setBookmarks(bms);
          setBmCount(bms.length);
        });
        return () => unsubBm();
  ...
  return (
    <>
      <style>{`
        ...
        .tn-shop-icon { transition: opacity 0.2s; }
        .tn-shop-icon:hover { opacity: 0.7; }
        .tn-shop-icon.active rect, .tn-shop-icon.active path { stroke: #fff; }
      `}</style>

      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isAdminPage ? 'bg-black' : scrolled ? 'bg-black/85 backdrop-blur-md' : 'bg-gradient-to-b from-black/60 to-transparent'
      }`}>
        {/* ✅ FULL-WIDTH: nggak dibatasi max-w-7xl lagi, cuma dikasih padding proporsional */}
        <div className="w-full mx-auto px-6 md:px-10 lg:px-14 2xl:max-w-[2000px]">
            ...
            {showSearch ? (
              <div className="flex items-center gap-2 flex-1">
                <input
                  ...
                  className="flex-1 bg-white/10 backdrop-blur text-white placeholder-white/50 border border-white/20 rounded-full px-5 py-2 lg:py-3 text-sm lg:text-base focus:outline-none focus:border-white/50"
                />
                <button onClick={handleSearchToggle} className="w-9 h-9 lg:w-11 lg:h-11 flex items-center justify-center text-white/80 hover:text-white">
              ...
              <>
                <Link to="/" className="hover:opacity-75 transition-opacity flex-shrink-0">
                  <img src="/IMG_20260220_144200.png" alt="Telier News" className="h-10 md:h-12 lg:h-14 w-auto object-contain" />
                </Link>

                <div className="flex items-center space-x-1 lg:space-x-2">
                  {isAdminPage ? (
                    isLoggedIn && (
                      <button onClick={handleLogout} className="px-4 py-2 lg:px-5 lg:py-2.5 text-sm lg:text-base font-medium border border-red-500 text-red-400 rounded-full hover:bg-red-500 hover:text-white transition-all">
                        Logout
                      </button>
                    )
                  ) : (
                    <>
                      {/* Search */}
                      <button onClick={handleSearchToggle} className="w-11 h-11 lg:w-12 lg:h-12 flex items-center justify-center text-white/90 hover:text-white transition-colors">
                        <svg className="w-5 h-5 lg:w-6 lg:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                      ...
                      <button
                        ...
                        className="w-11 h-11 lg:w-12 lg:h-12 flex items-center justify-center transition-colors"
                        title="Shop"
                        style={{ color: isShopPage ? '#ffffff' : 'rgba(255,255,255,0.9)' }}
                      >
                        <svg
                      ...
                      <button
                        ...
                        className="w-11 h-11 lg:w-12 lg:h-12 flex items-center justify-center text-white/90 hover:text-white transition-colors relative"
                      >
                        <svg className="w-5 h-5 lg:w-6 lg:h-6" fill={bmCount > 0 ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                      ...
                      {/* Profile */}
                      <button onClick={handleProfileClick} className="w-11 h-11 lg:w-12 lg:h-12 flex items-center justify-center text-white/90 hover:text-white transition-colors">
                        <svg className="w-5 h-5 lg:w-6 lg:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                      ...
                      {/* Menu */}
                      <button onClick={onMenuToggle} className="w-11 h-11 lg:w-12 lg:h-12 flex items-center justify-center text-white/90 hover:text-white transition-colors">
        ...
        >
          ...
          >
            ...
            <div style={{ padding: '20px 16px 60px' }}>
              {!isLoggedIn && (
                <div
                  onClick={() => { setShowBookmarks(false); setShowLoginPopup(true); }}
                  style={{
                ...
                >
                  <svg width="14" height="14" fill="none" stroke="#888" viewBox="0 0 24 24" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                  <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '12px', color: '#aaa' }}>
                    Login untuk simpan bookmark di semua device
                  </span>
                </div>
              )}
              ...
              {bookmarks.length === 0 ? (
                ...
                <>
                  ...
                  {bookmarks.map((bm) => (
                    <div key={bm.id} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', padding: '14px 0', borderBottom: '1px solid #0e0e0e' }}>
                      <img
                        src={bm.imageUrl} alt={bm.title}
      ...
      {/* Night mode button */}
      {isNewsDetail && (
        <div style={{ position: 'fixed', bottom: '80px', right: '20px', zIndex: 41, display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <button
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              cursor: 'pointer', color: nightMode ? '#000' : '#666', transition: 'all 0.2s',
            }}
          >
