...
const Header: React.FC<HeaderProps> = ({ onMenuToggle }) => {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/user-not-found') {
        setError('Akun tidak ditemukan. Klik Daftar untuk membuat akun.');
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        setError('Popup diblokir! Izinkan popup di browser.');
      } else {
  ...
  return (
    <>
      ...
      {/* POPUP LOGIN DENGAN FORM LENGKAP */}
      {showLoginPopup && (
        <div 
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold">
                {isSignUp ? 'Daftar' : 'Login'} TelierNews
              </h2>
              <button 
                onClick={closePopup}
            ...
            <p className="text-gray-600 text-sm mb-6">
              {isSignUp 
                ? 'Buat akun baru untuk akses fitur lengkap' 
            ...
            <div className="space-y-3">
              <button
                ...
                disabled={loading}
                className="w-full py-3 px-4 border border-gray-200 rounded-xl font-medium hover:bg-gray-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
              >
