...
const Header: React.FC<HeaderProps> = ({ onMenuToggle }) => {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 md:h-20">
            ...
            <Link to="/" className="hover:opacity-80 transition-opacity">
              <img
                src="/85615-removebg-preview (1).png"
                alt="Telier News"
            ...
            <div className="flex items-center space-x-2 md:space-x-3">
              {isAdminPage ? (
                isLoggedIn && (
                  <button onClick={handleLogout} className="px-4 py-2 text-sm font-medium border border-red-500 text-red-500 rounded-full hover:bg-red-500 hover:text-white transition-all">
                    Logout
                  </button>
                )
              ) : (
                <>
                  {/* Search icon */}
                  <button className="w-10 h-10 flex items-center justify-center text-white hover:text-gray-300 transition-colors" aria-label="Search">
                  ...
                  {/* Profile icon */}
                  <button onClick={handleProfileClick} className="w-10 h-10 flex items-center justify-center text-white hover:text-gray-300 transition-colors" aria-label={isLoggedIn ? 'Profile' : 'Login'}>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                  ...
                  {/* Hamburger menu */}
                  <button onClick={onMenuToggle} className="w-10 h-10 flex items-center justify-center text-white hover:text-gray-300 transition-colors" aria-label="Menu">
      ...
      {/* Spacer biar konten tidak ketutupan header fixed */}
      {isAdminPage && <div className="h-16 md:h-20" />}
