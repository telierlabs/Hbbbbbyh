...
const ArticleComments: React.FC<ArticleCommentsProps> = ({ articleId }) => {
  ...
  const handleDelete = async (commentId: string) => {
    ...
    try {
      ...
      const updated = myCommentIds.filter(id => id !== commentId);
      setMyCommentIds(updated);
      localStorage.setItem('tn_my_comments', JSON.stringify(updated));
    } catch (err) {
  ...
  return (
    <div style={{ marginTop: '40px', paddingTop: '32px', borderTop: '1px solid #f0f0f0' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '24px' }}>
        <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', letterSpacing: '0.3em', textTransform: 'uppercase', color: '#999' }}>Diskusi</span>
        {comments.length > 0 && (
          <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '8px', color: '#aaa', background: '#f5f5f5', padding: '2px 8px', borderRadius: '20px' }}>{comments.length}</span>
        )}
      </div>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '5px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '13px', fontWeight: 600, color: '#111' }}>{c.name || 'Anonim'}</span>
                    <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '9px', color: '#bbb' }}>{getRelativeTime(c.createdAt)}</span>
                  </div>
                  {canDelete(c) && (
                    <button onClick={() => handleDelete(c.id)} disabled={deletingId === c.id}
                      style={{ padding: '3px 8px', border: '1px solid #ffecec', background: '#fff5f5', color: '#ff6666', borderRadius: '6px', fontSize: '10px', fontWeight: 600, cursor: 'pointer', fontFamily: "'Montserrat', sans-serif", opacity: deletingId === c.id ? 0.5 : 1, flexShrink: 0 }}>
                      {deletingId === c.id ? '...' : 'Hapus'}
                    </button>
                  )}
                </div>
                <div style={{ background: '#f8f8f8', borderRadius: '0 12px 12px 12px', padding: '10px 14px' }}>
                  <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '13px', color: '#333', lineHeight: 1.6, margin: 0 }}>{c.text}</p>
                </div>
              </div>
            </div>
          ))}
